export const testimonials = [
  {
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    description:
      "Listing on this platform helped us reach thousands of daily commuters. The process was simple, and the response exceeded our expectations.",
    name: "ABC Foods",
    role: "Marketing Manager",
  },
  {
    avatarSrc: "/images/avatar/testimonials-2.jpg",
    description:
      "Earlier, finding clients for my billboard was tough. Now, I just list and let the platform bring in brands. Super convenient!",
    name: "Ayub Khan",
    role: "Hoarding Owner – Delhi",
  },
  {
    avatarSrc: "/images/avatar/testimonials-4.jpg",
    description:
      "The visibility and engagement we achieved from these hoardings were unmatched. The platform saved us time and effort in finding the right spots.",
    name: "TechNova",
    role: "Digital Lead",
  },
  {
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    description:
      "Within a week, we found multiple hoarding spaces in prime city areas. It’s now our go-to platform for outdoor advertising.",
    name: "UrbanStyle Clothing",
    role: "Brand Head",
  },
  {
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    description:
      "The platform connects us directly with serious advertisers. It has become an additional revenue stream for my property.",
    name: "Ramesh Kumar",
    role: "Hoarding Owner – Bangalore",
  },
];

export const testimonials2 = [
  {
    id: 1,
    avatar: "/images/avatar/testimonials-1.jpg",
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus sagittis, lorem in fringilla feugiat, nisi libero dapibus dui finibus luctus felis augue ac quam. Ut at egestas mauris. Nulla quis laoreet diam",
    name: "Brooklyn Simmons",
    location: "Los Angeles, CA",
  },
  {
    id: 2,
    avatar: "/images/avatar/testimonials-2.jpg",
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus sagittis, lorem in fringilla feugiat, nisi libero dapibus dui finibus luctus felis augue ac quam. Ut at egestas mauris. Nulla quis laoreet diam",
    name: "Leslie Alexander",
    location: "Los Angeles, CA",
  },
  {
    id: 3,
    avatar: "/images/avatar/testimonials-4.jpg",
    description:
      "Phasellus efficitur dictum urna, non elementum mauris mollis a. Duis lacinia arcu ac lorem ultricies porta. Maecenas eleifend malesuada egestas. In eu eros hendrerit, pharetra justo id, blandit orci.",
    name: "Ronald Richards",
    location: "Los Angeles, CA",
  },
  {
    id: 4,
    avatar: "/images/avatar/testimonials-3.jpg",
    description:
      "Sed eget nibh eros. Phasellus pharetra ac leo et auctor. Donec libero dui, blandit id nulla laoreet, pellentesque egestas ante. Etiam mollis tincidunt elit, sit amet sodales tellus finibus ut. Orci varius natoque.",
    name: "Annette Black",
    location: "Los Angeles, CA",
  },
  {
    id: 5,
    avatar: "/images/avatar/testimonials-3.jpg",
    description:
      "Phasellus efficitur dictum urna, non elementum mauris mollis a. Duis lacinia arcu ac lorem ultricies porta. Maecenas eleifend malesuada egestas. In eu eros hendrerit, pharetra justo id, blandit orci.",
    name: "Ronald Richards",
    location: "Los Angeles, CA",
  },
];

export const testimonials3 = [
  {
    id: 1,
    avatarSrc: "/images/avatar/testimonials-2.jpg",
    avatarWidth: 200,
    avatarHeight: 200,
    description:
      "Phasellus efficitur dictum urna, non elementum mauris mollis a. Duis lacinia arcu ac lorem ultricies porta. Maecenas eleifend malesuada egestas. In eu eros hendrerit, pharetra justo id, blandit orci.",
    name: "Annette Black",
    role: "CEO Themesflat",
  },
  {
    id: 2,
    avatarSrc: "/images/avatar/avt-png4.png",
    avatarWidth: 180,
    avatarHeight: 180,
    description:
      "Proin auctor, mauris at tristique ullamcorper, fi nunc nisi sagittis velit, eget blandit dolor arcu sed ante. Fusce euismod varius massa, id viverra nisl semper ut. Aliquam felis eros, eleifend non faucibus at.",
    name: "Annette Black",
    role: "CEO Themesflat",
  },
  {
    id: 3,
    avatarSrc: "/images/avatar/testimonials-4.jpg",
    avatarWidth: 200,
    avatarHeight: 200,
    description:
      "Sed eget nibh eros. Phasellus pharetra ac leo et auctor. Donec libero dui, blandit id nulla laoreet, pellentesque egestas ante. Etiam mollis tincidunt elit, sit amet sodales tellus finibus ut. Orci varius natoque.",
    name: "Annette Black",
    role: "CEO Themesflat",
  },
  {
    id: 4,
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    avatarWidth: 200,
    avatarHeight: 200,
    description:
      "Sed eget nibh eros. Phasellus pharetra ac leo et auctor. Donec libero dui, blandit id nulla laoreet, pellentesque egestas ante. Etiam mollis tincidunt elit, sit amet sodales tellus finibus ut. Orci varius natoque.",
    name: "Annette Black",
    role: "CEO Themesflat",
  },
  {
    id: 5,
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    avatarWidth: 200,
    avatarHeight: 200,
    description:
      "Phasellus efficitur dictum urna, non elementum mauris mollis a. Duis lacinia arcu ac lorem ultricies porta. Maecenas eleifend malesuada egestas. In eu eros hendrerit, pharetra justo id, blandit orci.",
    name: "Ronald Richards",
    role: "CEO Themesflat",
  },
];

export const slides = [
  {
    id: 1,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin rhoncus faucibus lacinia. Nunc ac augue facilisis, vestibulum dolor sed, blandit lectus. Sed ac dolor et eros suscipit.",
    avatar: "/images/avatar/avt-png12.png",
    name: "Courtney Henry",
    role: "Ceo & Founder at Themesflat",
  },
  {
    id: 2,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin rhoncus faucibus lacinia. Nunc ac augue facilisis, vestibulum dolor sed, blandit lectus. Sed ac dolor et eros suscipit.",
    avatar: "/images/avatar/avt-png12.png",
    name: "Courtney Henry",
    role: "Ceo & Founder at Themesflat",
  },
  {
    id: 3,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin rhoncus faucibus lacinia. Nunc ac augue facilisis, vestibulum dolor sed, blandit lectus. Sed ac dolor et eros suscipit.",
    avatar: "/images/avatar/avt-png12.png",
    name: "Courtney Henry",
    role: "Ceo & Founder at Themesflat",
  },
];

export const testimonials4 = [
  {
    id: 1,
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    description:
      "Proin auctor, mauris at tristique ullamcorper, nunc nisi sagittis velit, eget blandit dolor arcu sed ante. Fusce euismod varius massa, id viverra nisl semper ut. Aliquam felis eros, eleifend non faucibus at.",
    name: "Ronald Richards",
    role: "CEO Themesflat",
  },
  {
    id: 2,
    avatarSrc: "/images/avatar/testimonials-2.jpg",
    description:
      "Proin auctor, mauris at tristique ullamcorper, nunc nisi sagittis velit, eget blandit dolor arcu sed ante. Fusce euismod varius massa, id viverra nisl semper ut. Aliquam felis eros, eleifend non faucibus at.",
    name: "Courtney Henry",
    role: "CEO Themesflat",
  },
  {
    id: 3,
    avatarSrc: "/images/avatar/testimonials-4.jpg",
    description:
      "Proin auctor, mauris at tristique ullamcorper, nunc nisi sagittis velit, eget blandit dolor arcu sed ante. Fusce euismod varius massa, id viverra nisl semper ut. Aliquam felis eros, eleifend non faucibus at.",
    name: "Ronald Richards",
    role: "CEO Themesflat",
  },
  {
    id: 4,
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    description:
      "Proin auctor, mauris at tristique ullamcorper, nunc nisi sagittis velit, eget blandit dolor arcu sed ante. Fusce euismod varius massa, id viverra nisl semper ut. Aliquam felis eros, eleifend non faucibus at.",
    name: "Annette Black",
    role: "CEO Themesflat",
  },
  {
    id: 5,
    avatarSrc: "/images/avatar/testimonials-3.jpg",
    description:
      "Proin auctor, mauris at tristique ullamcorper, nunc nisi sagittis velit, eget blandit dolor arcu sed ante. Fusce euismod varius massa, id viverra nisl semper ut. Aliquam felis eros, eleifend non faucibus at.",
    name: "Ronald Richards",
    role: "CEO Themesflat",
  },
];

export const testimonials5 = [
  {
    rating: 5, // Number of stars
    description:
      "My experience with property management services has exceeded expectations. They efficiently manage properties with a professional and attentive approach in every situation. I feel reassured that any issue will be resolved promptly and effectively.",
    avatar: "/images/avatar/testimonials-4.jpg",
    name: "Annette Black",
    role: "CEO Themesflat",
    width: 200,
    height: 200,
  },
  {
    rating: 5,
    description:
      "Quisque tincidunt, nunc vitae maximus lobortis, tellus risus fringilla mi, pulvinar feugiat lacus ipsum nec tortor. Aliquam a venenatis orci, id bibendum eros.",
    avatar: "/images/avatar/avt-png7.png",
    name: "Eleanor Pena",
    role: "CEO Themesflat",
    width: 120,
    height: 120,
  },
  {
    rating: 5,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut aliquam tempus urna id interdum. Proin iaculis erat id sapien venenatis convallis. Nam et ullamcorper nibh. Nulla malesuada consectetur sem ut varius.",
    avatar: "/images/avatar/avt-png12.png",
    name: "Floyd Miles",
    role: "CEO Themesflat",
    width: 51,
    height: 51,
  },
  {
    rating: 5, // Number of stars
    description:
      "My experience with property management services has exceeded expectations. They efficiently manage properties with a professional and attentive approach in every situation. I feel reassured that any issue will be resolved promptly and effectively.",
    avatar: "/images/avatar/testimonials-4.jpg",
    name: "Annette Black",
    role: "CEO Themesflat",
    width: 200,
    height: 200,
  },
  {
    rating: 5,
    description:
      "Quisque tincidunt, nunc vitae maximus lobortis, tellus risus fringilla mi, pulvinar feugiat lacus ipsum nec tortor. Aliquam a venenatis orci, id bibendum eros.",
    avatar: "/images/avatar/avt-png7.png",
    name: "Eleanor Pena",
    role: "CEO Themesflat",
    width: 120,
    height: 120,
  },
  {
    rating: 5,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut aliquam tempus urna id interdum. Proin iaculis erat id sapien venenatis convallis. Nam et ullamcorper nibh. Nulla malesuada consectetur sem ut varius.",
    avatar: "/images/avatar/avt-png12.png",
    name: "Floyd Miles",
    role: "CEO Themesflat",
    width: 51,
    height: 51,
  },
  {
    rating: 5, // Number of stars
    description:
      "My experience with property management services has exceeded expectations. They efficiently manage properties with a professional and attentive approach in every situation. I feel reassured that any issue will be resolved promptly and effectively.",
    avatar: "/images/avatar/testimonials-4.jpg",
    name: "Annette Black",
    role: "CEO Themesflat",
    width: 200,
    height: 200,
  },
  {
    rating: 5,
    description:
      "Quisque tincidunt, nunc vitae maximus lobortis, tellus risus fringilla mi, pulvinar feugiat lacus ipsum nec tortor. Aliquam a venenatis orci, id bibendum eros.",
    avatar: "/images/avatar/avt-png7.png",
    name: "Eleanor Pena",
    role: "CEO Themesflat",
    width: 120,
    height: 120,
  },
  {
    rating: 5,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut aliquam tempus urna id interdum. Proin iaculis erat id sapien venenatis convallis. Nam et ullamcorper nibh. Nulla malesuada consectetur sem ut varius.",
    avatar: "/images/avatar/avt-png12.png",
    name: "Floyd Miles",
    role: "CEO Themesflat",
    width: 51,
    height: 51,
  },
  {
    rating: 5, // Number of stars
    description:
      "My experience with property management services has exceeded expectations. They efficiently manage properties with a professional and attentive approach in every situation. I feel reassured that any issue will be resolved promptly and effectively.",
    avatar: "/images/avatar/testimonials-4.jpg",
    name: "Annette Black",
    role: "CEO Themesflat",
    width: 200,
    height: 200,
  },
  {
    rating: 5,
    description:
      "Quisque tincidunt, nunc vitae maximus lobortis, tellus risus fringilla mi, pulvinar feugiat lacus ipsum nec tortor. Aliquam a venenatis orci, id bibendum eros.",
    avatar: "/images/avatar/avt-png7.png",
    name: "Eleanor Pena",
    role: "CEO Themesflat",
    width: 120,
    height: 120,
  },
  {
    rating: 5,
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut aliquam tempus urna id interdum. Proin iaculis erat id sapien venenatis convallis. Nam et ullamcorper nibh. Nulla malesuada consectetur sem ut varius.",
    avatar: "/images/avatar/avt-png12.png",
    name: "Floyd Miles",
    role: "CEO Themesflat",
    width: 51,
    height: 51,
  },
];
